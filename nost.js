        ...
        async function _processSingleCommand(commandLine) {
            ...
            switch (command) {
                ...
                case 'PART':
                    // PART 명령어는 이제 executeCommand에서 직접 처리됩니다 (녹화 시작).
                ...
                case 'EPART':
                    // EPART 명령어는 이제 executeCommand에서 직접 처리됩니다 (녹화 종료).
                    // _processSingleCommand로 올 일은 없습니다.
        ...
        async function executeCommand() {
            ...
            if (isWaitingForInput) {
                ...
                if (isRecordingBlock) {
                    if (command === 'EPART') {
                        if (parts.length < 2 || parts[1] !== currentBlockName) {
                            logToConsole("[ERROR] EPART command requires a matching PART block name. Current recording block: " + currentBlockName, 'error');
                            return;
                        }
                ...
                // --- Handle PART command to start recording ---
                if (command === 'PART') {
                    if (parts.length !== 2) {
                        logToConsole("[ERROR] PART command requires a <name>. E.g., PART myBlock", 'error');
                        return;
                    }
                    const blockName = parts[1];
                    if (codeBlocks.hasOwnProperty(blockName)) {
                        logToConsole(`[WARNING] Code block '${blockName}' already exists. It will be overwritten.`, 'system');
                    }
